...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  const resetUrl = `${req.protocol}://${req.get('host')}/api/resetPassword/${resetToken}` //Captura de protocolo

  const mensaje = `Hola\n
  Tu link para crear una nueva contraseña es el siguiente:\n
  ${resetUrl}\n\n
  Si no solicitaste este link, por favor comunicarte con soporte.\n\n
  Att: Cocodrilo Art-Gallery`

  try {
    await sendEmail({
      email: user.email,
      subject: 'Cocodrilo Art-Gallery. Recuperación de contraseña',
      mensaje
    })
    res.status(200).json({
      success: true,
...
exports.resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  const user = await User.findOne({
    resetPasswordToken,
    resetPasswordExpire: { $gt: Date.now() } //Fecha posterior en la que estoy en este momento
  })
  ...
  if (req.body.password !== req.body.confirmPassword) { //Nueva contraseña y confirmación de la nueva contraseña
...
exports.updatePassword = catchAsyncErrors(async (req, res, next) => {
  ...
  const sonIguales = await user.compararPass(req.body.oldPassword)//compararPass es el mismo de la linea 58 de models/auth.js
